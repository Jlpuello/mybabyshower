...
interface CreateMemoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (memory: MemoryRow) => void;
}
...
export const CreateMemoryModal = ({ isOpen, onClose, onSuccess }: CreateMemoryModalProps) => {
  ...
  const validate = (): boolean => {
    ...
    if (!mediaFile) errs.media = 'Debes subir una imagen o video';
    setErrors(errs);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.message || 'Error al subir el recuerdo');
      }

      const memory: MemoryRow = await res.json();
      onSuccess(memory);
      handleClose();
    } catch (err) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Agregar recuerdo" size="lg">
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-xs text-textLight">
              {isPublished ? (
                <><Eye className="w-3.5 h-3.5 text-green-500" /> <span className="text-green-600">Se publicará inmediatamente</span></>
              ) : (
          ...
          <div>
            ...
            {mediaPreview ? (
              ...
              >
                ...
                <p className="text-sm font-medium text-textSecondary">Subir foto o video</p>
        ...
        <div className="flex gap-3 pt-1 border-t border-gray-100">
          ...
          <Button type="submit" variant="primary" fullWidth disabled={loading || !title.trim() || !mediaFile}>
            {loading ? (
