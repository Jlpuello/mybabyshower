...
import { MoreHorizontal, UserPlus, Copy, Check, Ticket } from 'lucide-react';
...
const CopyCode = ({ code }: { code: string }) => {
  ...
  return (
    <button
      onClick={handleCopy}
      title="Copiar código"
      className="group flex items-center gap-1.5 font-mono text-sm text-textPrimary hover:text-goldAccent transition-colors"
    >
      <span>{code}</span>
...
export const GuestsTable = ({ guests, onGuestCreated }: GuestsTableProps) => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-ivory">
              <tr>
                {['Nombre', 'Código', 'Teléfono', 'Email', 'Estado', 'Acciones'].map((col) => (
                  <th
                    key={col}
              ...
              <AnimatePresence initial={false}>
                {guests.map((guest) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        className="text-textLight hover:text-textPrimary transition-colors rounded-lg p-1 hover:bg-warmBeige/50"
                        title="Más opciones"
                      >
